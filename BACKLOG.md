...
- **Scope-relative multi-verse links** *(done — feature/note-verse-linking)* — chapter/verse notes: `[1-11]`, `[1,5,7]`, `[1-11,15]`; book notes: `[1-11]` (chapters), `[3:1-11]`, mixed `[1-2,3:16]`. Pre-save render is clickable; save rewrites to `[v=…]`. Grammar: `ScopeRelativeLinkParser` + `app.js` helpers. Sermon notes stay absolute-ref only (no chapter scope).
...
- **Absolute multi-verse references in notes** — extend typed full refs so `[John 3:16-18]` / `[Jeremiah 13:1-11]` (and optional comma lists) normalize to multi-segment `[v=…]`. Scope-relative multi-verse is done; this is the book-qualified form for sermon notes and cross-book typing. Needs `ReferenceParser` (or sibling) range support + save normalize in reader + `/notes`.
