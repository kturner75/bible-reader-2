...
- **Save as passage from Insert expand** *(in progress)* — optional checkbox + title on the expand step; inserts `[v=…]` first, then fire-and-forget `POST /api/passages` (signed-in only).
