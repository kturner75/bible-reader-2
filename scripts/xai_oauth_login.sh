...
#
# SuperGrok OAuth is strictly for xAI API calls (VOTD, STT, upcoming TTS).
# It is not user sign-in and must not be mixed with Google OAuth.
#
# Do NOT share one refresh token with classic-chat-reader (CCR) prod.
# xAI rotates refresh tokens on every use; a shared token would invalidate
# the other deployment. Mint a separate token for rkj (this app).
#
...
# Ported from kturner75/classic-chat-reader — do not invent a second protocol.
#
# Requires: curl, jq
...
while (( SECONDS < deadline )); do
  ...
  if [[ -z "$error" ]]; then
    ...
    echo
    echo "Success. Set this on your rkj deployment (not CCR prod):"
    echo
